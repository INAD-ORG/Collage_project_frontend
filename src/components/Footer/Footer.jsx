  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        setMainCourses(allCourses.filter((course) => course.courseType === "Main Course"));
        setUgCourses(allCourses.filter((course) => course.courseType === "UG Course"));
        setPgCourses(allCourses.filter((course) => course.courseType === "PG Course"));
      } catch (err) {
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12 pb-12 sm:pb-16 border-b border-white/5">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <Link to="/" className="inline-block">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center p-1">
                  <img src={logo} alt="INAD" className="w-full h-full object-contain " loading="lazy" />
                </div>
                <div>
                  <h3 className="text-white font-semibold text-sm">International Academy</h3>
                  <p className="text-yellow-400 text-[10px] tracking-[0.2em] uppercase">Of Design</p>
                </div>
              </div>
            </Link>
            ...
            <div className="space-y-3">
              <a href={`tel:${contactDetailData.phoneNumber}`} className="flex items-center gap-3 text-white/50 hover:text-yellow-400 transition-colors group">
                ...
                <span className="text-sm">{contactDetailData.phoneNumber || "+91 70118 90082"}</span>
              </a>
              ...
              <div className="flex items-center gap-3 text-white/50">
                ...
                <span className="text-sm">{contactDetailData.email || "info@inad.edu"}</span>
              </div>
              ...
              <div className="flex items-start gap-3 text-white/50">
                ...
                <span className="text-sm leading-relaxed">{contactDetailData.address || "Sikar, Rajasthan"}</span>
              </div>
            </div>

            {/* Social Icons */}
            <div className="flex items-center gap-2 pt-2">
              <a href={contactDetailData.facebookLink} target="_blank" className="w-9 h-9 border border-white/10 flex items-center justify-center text-white/40 hover:bg-yellow-400 hover:text-black hover:border-yellow-400 transition-all duration-300">
                <FaFacebook className="text-sm" />
              </a>
              <a href={contactDetailData.instagramLink} target="_blank" className="w-9 h-9 border border-white/10 flex items-center justify-center text-white/40 hover:bg-yellow-400 hover:text-black hover:border-yellow-400 transition-all duration-300">
            ...
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li>
                  ...
                  >
                    <MdOutlineKeyboardArrowRight className={`transition-transform ${footerOption1 ? "rotate-90" : ""}`} />
                    UG Programs
                  </button>
                  {footerOption1 && (
                    <ul className="ml-4 mt-1 space-y-1">
                      {ugCourses?.map((item, index) => (
                        <li key={index}>
                          <Link to={`/course/${item.courseType}/${item._id}/${item.bannerTitle}`} className="text-white/30 hover:text-yellow-400 text-xs transition-colors">
                            {item.bannerTitle}
                          </Link>
                        </li>
                      ))}
                ...
                <li>
                  <button
                    ...
                    className="text-white/40 hover:text-yellow-400 text-xs transition-colors flex items-center gap-1 w-full text-left"
                  >
                    <MdOutlineKeyboardArrowRight className={`transition-transform ${footerOption2 ? "rotate-90" : ""}`} />
                    PG Programs
                  </button>
                  {footerOption2 && (
                    <ul className="ml-4 mt-1 space-y-1">
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/30 text-xs text-center sm:text-left">
            Copyright © {new Date().getFullYear()} International Academy of Design.
          </p>
